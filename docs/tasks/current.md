...
- 由技术负责人 T 先补上传异常失败态收口，再继续推进解析 worker 和规则资产加载器，实现 `upload_validated -> parsing -> parsed` 实链路

## 阻塞或待确认

- 上传接口和任务状态模型已完成第一轮收口，但在进入下一轮输入链路扩展前，仍需补齐上传异常时的 `failed` 状态收口
