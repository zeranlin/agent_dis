...
- `PDF` 与 `Word` 当前仅提供“文件读取 -> 文本解码 -> 结构切分”的最小文本解码占位实现
- 优先支持 UTF-8 和 `gb18030`
- 当前实现不等同于真实 `PDF` / `Word` 文件提取能力
- 首版不追求复杂版面还原，只保证输入链路占位可跑通
...
实现准备第二轮的目标不是把审查链路全部做完，而是把输入链路真正跑通。当前解析部分应明确理解为“最小文本解码占位实现”，而不是已经完成真实 `PDF` / `Word` 提取能力。只要解析 worker 和规则资产加载器稳定落地，下一轮就可以进入审查输入装配和审查执行实现。
