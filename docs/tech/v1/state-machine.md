...
- 上传成功后必须先写入 `created`
- 原始文件和文件元数据落地成功后，再推进到 `upload_validated`
...
- 若启用章节级初筛，则章节级初筛完成后才能进入条款级审查
- 当前最小代码实现暂未单独落章节级执行器，实际先走 `review_queued -> reviewing_clauses -> aggregating`
- 条款级审查结束后才允许开始汇总

### 6.4 汇总规则

- 至少存在结构化风险结果或明确的“未发现明显风险”结果，才能结束汇总
- 最终结论、审查报告和 `review_result` 结果对象都生成成功，才可进入 `completed`
...
| 上传落地 | 原始文件存储失败、文件元数据写入失败 | 若任务已创建，则任务置为 `failed`，记录错误信息 |
...
V1 采用“对外少状态、对内细状态”的状态机设计。上传阶段的统一口径是：先写 `created`，再在文件和元数据落地完成后推进到 `upload_validated`。这样既能保持页面和接口简单，也能满足开发排障和后续重试控制的需要。
